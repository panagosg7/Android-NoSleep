...
public class ContextSensitiveLocking {
   ...
   */
  private ExplodedInterproceduralCFG icfg;
  ...
  public ContextSensitiveLocking(ExplodedInterproceduralCFG icfg) {
    /*
     * We already have created our SensibleExplodedInterproceduralCFG which
     * contains extra nodes compared to the exploded i-cfg WALA usually creates
     */
   ...
   * acquired, maybe released, must be released)
   * 
   * @author pvekris
   */
  ...
  private class LockingFunctions implements IPartiallyBalancedFlowFunctions<BasicBlockInContext<IExplodedBasicBlock>> {
    ...
    public IUnaryFlowFunction getNormalFlowFunction(
       ...
       * acquire/release is involved.
       * BE CAREFUL : exceptional edges that span across multiple 
       * procedures (if possible) cannot be determined. 
       */      
      /*
      if (isExceptionalEdge(src, dest)) {
        E.log(1, "Killing [" + src.toString() + " -> " + dest.toString() +"]");
    ...
    /**
     * Flow function from call node to return node at a call site when callees
     * exist.
     */
    @Override
    ...
    private boolean isExceptionalEdge(
      ...
      if (icfg.getCGNode(src).equals(icfg.getCGNode(dest))) {
        ControlFlowGraph<SSAInstruction, IExplodedBasicBlock> cfg = icfg.getCFG(src);
    ...
    private Collection<PathEdge<BasicBlockInContext<IExplodedBasicBlock>>> collectInitialSeeds() {
      ...
      for (BasicBlockInContext<IExplodedBasicBlock> bb : supergraph) {
        if (isWLAcquireCall(bb)) {
          ...
          // note that the fact number used for the source of this path edge
