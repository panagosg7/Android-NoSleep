...
public abstract class IR {
  ...
  private Map<Integer, ISSABasicBlock> instructionIndex2Block;
  ...
  /*
   * Panagiotis Vekris added these two methods
   */
  public ISSABasicBlock getBasicBlockForInstructionIndex(int s) {
    if (instructionIndex2Block == null) {
      mapInstructionIndex2Blocks();
    }
    return instructionIndex2Block.get(s);
  }

  private void mapInstructionIndex2Blocks() {
    instructionIndex2Block = HashMapFactory.make();
    for (ISSABasicBlock b : cfg) {
      for (int i = b.getFirstInstructionIndex(); 
          i<= b.getLastInstructionIndex();
          i++) {
        instructionIndex2Block.put(i, b);
      }
    }
  }
