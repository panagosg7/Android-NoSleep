 ...
 */
public class CFGDotUtil {

  /**
   * Customize the dot output:
   * 
   */
  
  private static boolean exportPDF = false;  
  ...
  /**
   */
  public static <T> void dotify(SSACFG g, NodeDecorator labels, String dotFile, String outputFile, String dotExe)
      throws WalaException {   
    
    dotify(g, labels, null, dotFile, outputFile, dotExe);
  }

  public static <T> void dotify(SSACFG g, NodeDecorator labels, String title, String dotFile, String outputFile, String dotExe)
      throws WalaException {
    if (g == null) {
  ...
  public static void spawnDot(String dotExe, String outputFile, File dotFile) throws WalaException {
    ...
    try {
      ...
      e.printStackTrace();
      throw new WalaException("IOException in " + CFGDotUtil.class);
    } finally {
      if (output != null) {
        try {
          output.close();
   ...
   * @throws WalaException
   */
  private static <T> StringBuffer dotOutput(SSACFG g, NodeDecorator labels, String title) throws WalaException {
    ...
    for (Iterator<ISSABasicBlock> it = g.iterator(); it.hasNext();) {
      ISSABasicBlock n = it.next();
      for (Iterator<ISSABasicBlock> it2 = g.getSuccNodes(n); it2.hasNext();) {
        ISSABasicBlock s = it2.next();
        String nstr = getStringForNode(n);
        String sstr = getStringForNode(s);
        if (nstr!=null && sstr!=null) {
          result.append("\"");
          result.append(nstr);
          result.append("\"");
          result.append(" -> ");          
  ...
  private static void outputNodes(NodeDecorator labels, StringBuffer result, Collection<ISSABasicBlock> dotNodes) throws WalaException {
    for (Iterator<ISSABasicBlock> it = dotNodes.iterator(); it.hasNext();) {
        outputNode(labels, result, it.next());
    }
  }

  
  private static void outputNode(NodeDecorator labels, StringBuffer result, ISSABasicBlock n) throws WalaException  {
    String str = getStringForNode(n);
    if (str != null) {
      result.append("\"" + str + "\"\n");
  ...
  private static String getStringForNode(ISSABasicBlock n) {
    try {
      
      return n.getLastInstruction().toString();
   ...
   */
  private static HashSet<ISSABasicBlock> computeDotNodes(SSACFG g) throws WalaException {
    HashSet<ISSABasicBlock> blocks = new HashSet<ISSABasicBlock>();
    Iterator<ISSABasicBlock> itr = g.iterator();
    while (itr.hasNext()) {
      blocks.add(itr.next());      
    }
    return blocks;
